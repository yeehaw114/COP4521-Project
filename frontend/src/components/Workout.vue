<template>
    <v-card class="mx-auto pa-6" elevation="15" max-width="800" rounded="lg">
            ...
            <v-col v-for="(s, si) in e.sets" :key="si" cols="6">
              <div class="py-3">
                <v-card class="mx-auto pa-3" elevation="3" max-width="300" rounded="md">
                  ...
                  >{{ si + 1 }}. Reps: {{ s.reps }}<v-icon icon="mdi-counter"></v-icon> Weight: {{ s.weight }}<v-icon icon="mdi-weight-pound"></v-icon></div>
                </v-card>
              </div>
            </v-col>
          </v-row>
      ...
      <v-btn class="mb-8" color="green" size="large" variant="tonal" link :to="`/workout/${props.workout.id}/log`" block>Log</v-btn>
    </v-card>
  </template>
